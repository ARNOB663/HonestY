...
// "Order received" email fires immediately on placement. It is NOT the
// confirmation — that goes out separately when the admin clicks Confirm and
// the status transitions to "confirmed" / "paid" (sendStatusUpdate).
export async function sendOrderConfirmation(order) {
  const short = String(order._id).slice(-6);
  const addr = order.shippingAddress || {};
  const pay = order.payment || {};
  const payLine = pay.method === "cod"
    ? "Pay on delivery (Cash). We'll confirm your order shortly."
    : `Paid via ${pay.method?.toUpperCase()} — TrxID <code>${esc(pay.txnId)}</code>. We'll verify and confirm shortly.`;
  // Include both id+email so /track auto-fills and looks up the order on
  // arrival — the customer never has to type anything.
  const trackUrl = `${getBaseUrl()}/track?id=${order._id}${order.userEmail ? `&email=${encodeURIComponent(order.userEmail)}` : ""}`;
  const body = `
    <p>Thanks ${esc(addr.name || "")}, we&rsquo;ve received your order <strong>#${short}</strong>.</p>
    <p style="color:#666;font-size:14px;">${payLine}</p>
    ${itemsTable(order.items, order)}
    <p style="margin-top:20px;font-size:14px;"><strong>Ship to:</strong><br>${esc(addr.name)}<br>${esc(addr.line1)}<br>${esc(addr.city)}, ${esc(addr.state)}<br>${esc(addr.phone)}</p>
  ...
  return send({
    to: order.userEmail,
    subject: `Order #${short} received — ${BRAND}`,
    html: layout(`Order received`, body),
    text: `We've received your order #${short}. Total ${formatMoney(order.total)}. Track: ${trackUrl}`,
  });
}

// Status copy depends a bit on payment method. The "paid" status fires when
// an admin confirms an order — for prepaid (bKash/Nagad) that's "Payment
// received"; for COD it's "Order confirmed" because the customer hasn't
// paid yet (they pay the courier on delivery).
function statusCopy(status, method) {
  const isCod = method === "cod";
  switch (status) {
    case "confirmed":
      return { title: "Order confirmed", line: isCod
        ? "We've confirmed your order. Pay the courier in cash when it arrives."
        : "We've confirmed your order. We're preparing it for shipment." };
    case "paid":
      return isCod
        ? { title: "Payment received", line: "We've received your payment. Thanks!" }
        : { title: "Payment received", line: "Your payment is confirmed. We're preparing your order." };
    case "fulfilled":
      return { title: "Order packed", line: "Your order has been packed and is awaiting pickup by the courier." };
    case "shipped":
      return { title: "Order shipped", line: "Your order is on the way." };
    case "delivered":
      return { title: "Order delivered", line: "Your order has been delivered. Enjoy!" };
    case "cancelled":
      return { title: "Order cancelled", line: "Your order has been cancelled. If this is unexpected, please reply to this email." };
    case "refunded":
      return { title: "Refund issued", line: "We've issued a refund for your order." };
    default:
      return null;
  }
}
...
export async function sendStatusUpdate(order, status) {
  const copy = statusCopy(status, order?.payment?.method);
