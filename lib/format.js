...
// Substitutes {tokens} in admin-editable copy with live store settings, so the
// announcement bar and trust badges can't drift out of sync with the actual
// free-shipping threshold. Unknown tokens are left untouched rather than blanked
// — a typo shows up as literal text instead of silently vanishing.
export function applyStoreTokens(text, settings) {
  if (!text) return "";
  const tokens = {
    freeshipping: () => formatMoney(Number(settings?.freeShippingThreshold) || 0),
    storename: () => String(settings?.storeName || ""),
    homedivision: () => String(settings?.homeDivision || ""),
  };
  return String(text).replace(/\{(\w+)\}/g, (match, name) => {
    const fn = tokens[String(name).toLowerCase()];
    return fn ? fn() : match;
  });
}
