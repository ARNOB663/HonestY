...
// Clean spec rows: drop empty rows, cap row count and field length.
export function sanitizeSpecs(raw) {
  if (!Array.isArray(raw)) return [];
  return raw
    .slice(0, 40)
    .map((s) => ({
      key: typeof s?.key === "string" ? s.key.trim().slice(0, 80) : "",
      value: typeof s?.value === "string" ? s.value.trim().slice(0, 500) : "",
    }))
